...
import type { Job, JobMatch, RecruiterMessageChannel } from "@/types/database";
...
export function ApplicationKit({
  ...
  const [coverLetter, setCoverLetter] = useState<{ id: string; content?: string } | null>(null);
  ...
  async function generateCoverLetter() {
    ...
    try {
      ...
      if (!res.ok) {
        setCoverLetterError(data.error ?? "Could not generate cover letter.");
        return;
      }
      setCoverLetter({ id: data.coverLetterId });
    } catch {
      setCoverLetterError("Could not reach the AI service.");
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      {resumes.length > 1 && (
        <Card className="p-4">
          <label className="mb-1 block text-sm font-medium text-slate-700">Building this kit with</label>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card className="p-5">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-900">
            <Check className="h-4 w-4 text-emerald-600" /> CV
          </h2>
          <p className="mt-2 text-sm text-slate-600">
            Tailor your CV&apos;s summary and skills to this specific role before applying.
          </p>
          <Link href={resumeId ? `/dashboard/resumes/${resumeId}?job=${job.id}` : "/dashboard/resumes"}>
        ...
        <Card className="p-5">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-900">
            {coverLetter ? <Check className="h-4 w-4 text-emerald-600" /> : <Sparkles className="h-4 w-4 text-brand-600" />}
            Cover Letter
          </h2>
          {!coverLetter && (
            <>
              <p className="mt-2 text-sm text-slate-600">
                Generate a cover letter tailored to this role from your real experience.
              </p>
              <Button
                type="button"
                variant="outline"
                size="sm"
                className="mt-3"
                disabled={coverLetterPending || !resumeId}
                onClick={generateCoverLetter}
              >
                {coverLetterPending ? "Generating…" : "Generate Cover Letter"}
              </Button>
            </>
          )}
          {coverLetter && (
            <>
              <p className="mt-2 text-sm text-emerald-700">Cover letter ready.</p>
          ...
          {coverLetterError && <p className="mt-2 text-sm text-red-600">{coverLetterError}</p>}
        </Card>
